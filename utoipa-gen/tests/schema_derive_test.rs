...
// Regression test for https://github.com/juhaku/utoipa/issues/1454.
//
// The generated `ToSchema` / `IntoParams` code used to build the list of
// properties / parameters as a stack allocated array literal (`[a, b, ...]`).
// For a type with a large number of fields that array is large enough to
// trigger `clippy::large_stack_arrays` and, in the worst case, overflow the
// stack at runtime. The generated code now uses a heap allocated `vec![...]`
// instead, so deriving on a "wide" type must keep compiling and producing a
// valid schema.
#[test]
